...
#[rustfmt::skip]
fn bose_nelson_sort_5(arr: &mut [u8; 5]) {
    if arr[0] > arr[1] { arr.swap(0, 1); }
    if arr[3] > arr[4] { arr.swap(3, 4); }
    if arr[2] > arr[4] { arr.swap(2, 4); }
    if arr[2] > arr[3] { arr.swap(2, 3); }
    if arr[1] > arr[4] { arr.swap(1, 4); }
    if arr[0] > arr[3] { arr.swap(0, 3); }
